#!/usr/bin/env python3
"""
Download genomic (MAF) and clinical data from GDC for TCGA-BRCA patients.

Usage:
    python data/download_genomic_brca.py
"""
import csv
import gzip
import json
...
IMAGING_METADATA = Path("/hpcwork/p0021834/datasets/TCGA-BRCA/metadata.csv")
...
def get_target_patients():
    ...
    Uses the clinical CSV (all TCGA-BRCA patients) so that every patient with
    ...
    if IMAGING_METADATA.exists():
        ...
        col = "PatientID" if "PatientID" in df.columns else "Subject ID"
        log.info("Using imaging metadata: %d patients", len(df[col].unique()))
        return sorted(df[col].unique().tolist())
    log.warning("No clinical or imaging metadata — downloading for ALL BRCA patients")
    return None
...
def download_clinical(patient_ids):
    ...
    resp = gdc_post(GDC_CASES, {"filters": filters, "fields": ",".join(fields), "size": 1200})
...
if __name__ == "__main__":
    # Download clinical first (for all TCGA-BRCA), then use those patient IDs for MAFs.
